...
seedreduce = [1204, 3298, 17502002, 1813315, 123456] # seeds for sampled runs
...
def PPinput(fulldf):
    ...
    myinput.mB = fulldf.mB #- 0.24# - fulldf.biasCor_mu
...
if len(seedreduce) == 0:
    ...
    if reducelowz:
        ...
        print(subinp.shape, jla.shape, allinp.shape)
...
p = np.full(l, 1/l)
m = 1000
for seed in seedreduce:
    np.random.seed(seed)
    choice = np.unique(np.random.choice(idx, size = m, replace=False, p=p))
    dfsamp = allinp.loc[np.array(allinp.index)[choice]]
