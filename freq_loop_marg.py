...
def runfreq(prefixinp, prefixsav, zC=np.linspace(0,0.1,41)):
    ...
    #zmin = 0.0  # Redshift cut
        ...
        def GetMaxLikelihood(model, ip, H0, zmin = zmin, Z = Z, COVd = COVd, tolerance = tlrc):
            ...
            def RES( A , B , M0, X0, C0 , *cosm_params , deltamu0=0): # Residual
                #print(*cosm_params)
                ...
                if model == 'Standardmodel':
                    mu = mu[0]
                return np.hstack( [ (Z[i,1:4] -np.array([mu[i] - deltamu0,0,0]) - Y0A ) for i in range(N) ] )  
            
            
            def m2loglikedeltamu0(pars , cov_params_ = cov_params, res_params_ = res_params, deltamu0=0): # -2 * Ln(Likelihood)
                ...
                try:
                    ...
                    return 13995*10.**20
                res = RES( *[ pars[i] for i in res_params_ ] , deltamu0=deltamu0)
            ...
            def integrationcenter( A , B , M0, X0, C0 , *cosm_params ):
                return M0-A*X0+B*C0
            
            def integrationwidth( A , B , VM, VX, VC ):
...
#         print('LCDM     :', res_lambdaCDM[0].success, res_lambdaCDM[0].message, 'result:', res_lambdaCDM[0].x)
#         print('Milne    :', res_lambdaCDM[1].success, res_lambdaCDM[1].message, 'result:', res_lambdaCDM[1].x)
#         print('Timescape:', res_timescape.success, res_timescape.message, 'result:', res_timescape.x)
...
#     name = sys.argv[1] + '_'
#     prefix = 'Pantheon/Build/' + name
#     outputprefix = prefix
#     print(prefix, outputprefix)
#     rundistmod(prefix)
#     print('finished distmod in {0:.02f} minutes'.format((time.time() - start)/60))
#     runfreq(prefix, outputprefix, [0.01, 0.033, 0.05, 0.075, 0.1])
#     print('finished frequentist in {0:.02f} minutes'.format((time.time() - start)/60))

    # Nseeds = 50
    # Nsamples = [1000]#[580, 660, 750, 875, 1000]
    zcuts = [0.03]#np.linspace(0,0.1,41)#[0.01, 0.033, 0.05, 0.075, 0.1]
    # print('start:', Nseeds, 'seeds | sample sizes:', Nsamples, '| redshift cuts:', zcuts)
    # for m in Nsamples:
    #     for seed in range(Nseeds):
    #         prefix = 'Pantheon/Build/random1000x50/PP_1690random' + str(m) + '_' + str(seed) + '_'
    #         print('\n', prefix)
    #         rundistmod(prefix)
    #         enddist = time.time()
    ...
    runfreq('Pantheon/Build/PP_1690jla_', 'Pantheon/Build/PP_1690jlamarg_', zcuts)
