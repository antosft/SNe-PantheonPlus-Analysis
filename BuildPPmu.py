...
allcov = pd.concat(covsyst + covmu + [covfit, covdupl], axis=1).groupby(level=0, axis=1).sum() # sum over all psi of Eq. (7) to get full covariance, add statistical & duplication covariances
allcov = allcov.loc[idxcov, colcov]

# -------------------- save ----------------------------------------------------------
print('save')
np.savetxt('Pantheon/Build/PP_fitmu_COVd.txt', np.array(allcov))
np.savetxt('Pantheon/Build/PP_fitmu_input.txt', np.array(allinp))
