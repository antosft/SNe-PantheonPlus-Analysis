# -*- coding: utf-8 -*-
"""
Created on Fri Jan 26 16:01:15 2024
...
versionname = sys.argv[1] # base sample from which the data was generated; for Nseeds == 0, 'PP_' + versionname + '_input.txt' etc. are used
...
def runfreq(prefixinp, prefixsav, zC=np.linspace(0,0.1,41)):
    ...
    for zmin in zC:
        ...
        def GetMaxLikelihood(model, ip, H0, zmin = zmin, Z = Z, COVd = COVd, tolerance = tlrc):
            ...
            # ===================== EXTRACT mB COV MATRIX (N x N) =======================
            COVd = COVd[::3,::3]     # extracts cov matrix of the larger matrix
            # ===========================================================================     
            ...
            if model == 'Standardmodel': 
                ...
                cov_params = [2,3,5]	# Index of parameters for covariance matrix propagation 
                ...
                cov_params = [1,2,4]	# Index of parameters for covariance matrix propergation 
                res_params = [1,2,4,0]	# Index of parameters for residual calculation 
            # ============================================================================================
            ...
            def COV( A , B , VM): # Covariance matrix of final expression for Likelihood function
                # block3 = np.array( [[VM + VX*A**2 + VC*B**2,    -VX*A, VC*B],
        	       #                  [-VX*A , VX, 0],
        	       #                  [ VC*B ,  0, VC]] )
                ATCOVlA = np.identity(N) * VM #linalg.block_diag( *[ block3 for i in range(N) ] )
                return np.array( COVd + ATCOVlA )
        	
            
            def RES( A , B , M0, *cosm_params ): # Residual
                #print(*cosm_params)
                Y0A = np.array([ M0-A*Z[i, 2]+B*Z[i, 3] for i in range(N) ]) 
                mu = MU(*[ cosm_params[i] for i in range(len(cosm_params)) ]) ;
                if model == 'Timescape':
                    mu = MU(cosm_params[0] )
                if model == 'Standardmodel':
                    mu = mu[0]
                Zhat = np.array( [ (Z[i,1] - mu[i]) for i in range(N) ])
                return Zhat - Y0A
            ...
            def m2loglike(pars , cov_params_ = cov_params, res_params_ = res_params): # -2 * Ln(Likelihood)
                ...
                if model == 'Standardmodel':
                    bool_pars = pars[0]<0 or pars[0]>1.5 or pars[1]<-.50 or pars[1]>1.5 or pars[5]<0
                elif model == 'Timescape':    
                    bool_pars = pars[0]< 0.001 or pars[0]> 0.99 or pars[4]<0
            ...
            # order of parameters: OM, OL, alpha, beta, M, V_M
            #if zmin > 0.04:
            ...
            pre_found_empty = np.array([  5.16659541e-11,   5.18656056e-06,   0.14753,
                ...
                -1.89983801e+01,
                 1.07146542e-02])  
            # order of parameters: OM, alpha, beta, M, V_M
            pre_found_timescape = np.array([  3.27194377e-01,   0.14753,   
                3.09220,  
                -1.92988653e+01,   1.07390412e-02])
            # else:
