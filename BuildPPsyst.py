...
covsyst = [PPcovSYST(dfs[fo], dfs[nominal], summary.loc[fo, 'weights']) for fo in np.setdiff1d(summary.index, [nominal])]
...
allcov = pd.concat(covsyst + [covfit], axis=1).groupby(level=0, axis=1).sum() # sum over all psi of Eq. (7) to get full covariance, add statistical covariance
allcov = allcov.loc[idxcov, colcov]

# -------------------- save ----------------------------------------------------------
print('save')
np.savetxt('Pantheon/Build/PP_syst_COVd.txt', np.array(allcov))
np.savetxt('Pantheon/Build/PP_syst_input.txt', np.array(allinp))
